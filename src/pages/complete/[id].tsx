import { Box, Button, Container, Flex, Heading, Radio, RadioGroup, Select, Stack, Table, TableCaption, TableContainer, Tbody, Td, Th, Thead, Tr } from '@chakra-ui/react';
...
const CompleteId = () => {
  ...
  const pdhDownloadHandler = () => {
    ...
    html2canvas(target, { scale: 2.5 }).then((canvas) => {
      ...
      let pdf = new jsPDF();
      pdf.addImage(imgData, 'SVG', 5, 10, canvas.width / 10, canvas.height / 10);
      pdf.save(`${serialNumber}.pdf`);
    });
  };

  const handleInputChange = (e: React.ChangeEvent<HTMLSelectElement> | string, prop = "") => {
    if (typeof e === 'string') {
    ...
    <Box w="100%" mt={12} px={6}>
      ...
      >
        <Flex w="full" flexDirection="column" alignItems="center" fontSize="xl">
          ...
          <Box mt={3} fontSize="sm">発注No.{getSerialNumber(Number(serialNumber))}</Box>
          <Flex p={6} mt={3} flexDirection="column" alignItems="flex-start" border="1px" borderColor="gray.200">
            <Flex>
              <Box>品番：{product?.productNumber}</Box><Box ml={3}>{product?.productName}</Box>
            </Flex>
      ...
      <Container
        ...
        boxShadow="md"
      >
        <Heading as="h2" fontSize="lg">発注書を作成する</Heading>
        <RadioGroup
          mt={6}
          p={3}
          name="orderType"
        ...
        <Flex justifyContent="center">
          <Button mt={6} colorScheme='facebook' onClick={pdhDownloadHandler}>PDF作成</Button>
        </Flex>
      </Container>
      <Container
        id="pdf-id"
      ...
      >
        <Stack spacing={6}>
          ...
          <Box textAlign="center" fontSize="3xl">{items.orderType}</Box>
          <TableContainer >
