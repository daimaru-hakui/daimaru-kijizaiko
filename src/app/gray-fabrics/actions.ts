...
export async function orderGrayFabricAction(
  ...
  const serialNumberRef = db.collection('serialNumbers').doc('grayFabricOrderNumbers')

  try {
    await db.runTransaction(async (transaction) => {
      const serialSnap = await transaction.get(serialNumberRef)
      if (!serialSnap.exists) throw new Error('serialNumbers does not exist')
      ...
      const newSerial = (serialSnap.data()!.serialNumber as number) + 1
      transaction.update(serialNumberRef, { serialNumber: newSerial })
