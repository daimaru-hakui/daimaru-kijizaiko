...
function NavItemLink({
  ...
  return (
    <Link
      ...
      onClick={handleClick}
      className={cn(
        "block px-3 py-1 rounded text-sm hover:bg-slate-100 transition-colors text-slate-700",
...
export function NavLinks({ roles, pathname, onClose }: Props) {
  ...
  return (
    // 区切り線 (hr) が -mx-3 で左右のパディング分だけ広がるため、px-3 は左右対称に
    // しておく (pr-0 を足すと右だけ 12px はみ出して横スクロールバーが出る)。
    // 念のため x 軸のスクロールも殺しておく
    <nav className="overflow-y-auto overflow-x-hidden h-full py-6 space-y-3 px-3 w-full">
      <Link
        href="/dashboard"
